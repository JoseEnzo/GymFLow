...
export function useAuth() {
  ...
  async function signUp(email: string, password: string, fullName: string, accountType: 'owner' | 'personal' | 'student' = 'owner', redirectTo?: string, document?: string, phone?: string) {
    ...
    const { data, error } = await supabase.auth.signUp({
      ...
      password,
      options: {
        data: { full_name: fullName, account_type: accountType, document: document ? document.replace(/\D/g, '') : null, phone: phone ? phone.replace(/\D/g, '') : null },
        emailRedirectTo,
      },
    })
    if (error) throw error
    ...
    if (data.session) {
      ...
      // Supabase email confirmation is required — the confirmation link uses emailRedirectTo
