...
function LoginInner() {
  ...
  function handleRoleSelect(r: Role) {
    ...
    setInviteToken(null)
    reset()
    // Skip invite-code step when arriving from a URL invite (redirect already targets /convite/)
    const comingFromInviteUrl = redirect?.startsWith('/convite/')
    setStep((r === 'personal' || r === 'student') && !comingFromInviteUrl ? 'invite' : 'form')
  }
  ...
  async function onSubmit(data: FormData) {
    ...
    try {
      ...
      await signIn(email, data.password, effectiveRedirect, role ?? undefined)
    } catch (err: unknown) {
