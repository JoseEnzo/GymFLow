...
export default function ConvitePage() {
  ...
  async function loadInvite() {
    setStatus('loading')
    try {
      // Use server-side API (service role) to bypass RLS for anonymous users —
      // direct Supabase queries do an INNER JOIN that RLS silently drops the row for anon.
      ...
      if (!res.ok) throw new Error(json.error ?? 'Convite não encontrado ou expirado')

      const inviteDetails: InviteDetails = {
        academyName: json.academyName!,
        academySlug: json.academySlug!,
        role: json.role!,
        code: json.code!,
