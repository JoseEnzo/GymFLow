    ...
    <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4">
      ...
      <div className="w-full max-w-lg">
        <AnimatePresence mode="wait">
          ...
          {step === 'role' && (
            <motion.div key="role" variants={slide} initial="hidden" animate="show" exit="exit" className="space-y-6">
              ...
              <div className="grid gap-3">
                ...
                <button
                  onClick={() => { setRole('personal'); setStep('invite') }}
                  className="flex items-center gap-4 p-5 rounded-2xl border border-border/60 hover:border-brand-500/40 hover:bg-brand-500/5 text-left transition-all duration-200 group"
                >
                  <div className="w-12 h-12 rounded-2xl flex items-center justify-center bg-brand-500/15 flex-shrink-0 group-hover:scale-105 transition-transform">
          ...
          {/* ── STEP: invite choice (student / personal) ── */}
          {step === 'invite' && (
            <motion.div key="invite" variants={slide} initial="hidden" animate="show" exit="exit" className="space-y-6">
              <div>
                {role === 'personal' ? (
                  <>
                    <h1 className="text-2xl font-display font-bold">Entre com seu convite 🔑</h1>
                    <p className="text-muted-foreground mt-1.5 text-sm">
                      Para trabalhar como personal no GymFlow você precisa de um convite da academia. Peça ao proprietário para gerar um código.
                    </p>
                  </>
                ) : (
                  <>
              ...
              {/* Personal: only invite input */}
              {role === 'personal' ? (
                <div className="space-y-3">
                  <div className="flex items-start gap-3 px-4 py-3 rounded-xl bg-violet-500/10 border border-violet-500/20">
                    <Ticket className="w-4 h-4 text-violet-400 flex-shrink-0 mt-0.5" />
                    <p className="text-xs text-violet-300/80 leading-relaxed">
                      O código de convite é enviado pelo proprietário da academia pelo painel de Personais.
                    </p>
                  </div>
                  <div className="space-y-1.5">
