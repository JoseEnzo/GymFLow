...
function OnboardingContent() {
  ...
  async function saveAcademy() {
    ...
    try {
      ...
      const { error: academyError } = await (supabase.from('academies') as any).insert({
        id: newId,
        owner_id: user.id,
        name: academyName,
        slug,
      ...
      // Agora o usuário é membro — RLS de SELECT passa
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const { data: newAcademy, error: fetchError } = await (supabase.from('academies') as any)
        .select('*')
        .eq('id', newId)
        .single()
