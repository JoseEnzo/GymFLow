...
- **Domínio canônico migrado pra `www.meutrein.com.br` (jun/2026).** O domínio foi comprado no Registro.br, DNS (A apex `216.198.79.1` + CNAME www → vercel-dns) e SSL Let's Encrypt já configurados e no ar. Os fallbacks de `NEXT_PUBLIC_APP_URL` em `app/layout.tsx` (`metadataBase` + `openGraph.url`), `app/sitemap.ts`, `app/robots.ts`, `app/api/cron/notifications/inactivity` e o pill da `app/opengraph-image.tsx` agora usam `www.meutrein.com.br` (antes `gymflow.app`). **Pré-requisito de prod:** setar `NEXT_PUBLIC_APP_URL=https://www.meutrein.com.br` no painel Vercel (Production) + redeploy — sem isso, robots/sitemap/OG saem com a URL de preview da Vercel. **Ainda em `gymflow.app`:** os e-mails de contato (`contato@`, `privacidade@`) e o handle `@gymflow` do Twitter (dependem de e-mail/social próprios, não trocados).
...
`apps/web/components/layout/billing-banner.tsx`, montado no `(dashboard)/layout.tsx` acima do conteúdo. Só `role === 'owner'` vê; oculto em `/configuracoes`. Estados: sem `subscription_status` + plano pago + sem `stripe_subscription_id` (checkout abandonado, âmbar), `past_due` (vermelho), `canceled` (vermelho), `trialing` + `trial_ends_at` faltando ≤3 dias (vermelho, "teste grátis acabando" — jun/2026). CTA → `/configuracoes?tab=plano`. **Não bloqueia o uso** — paywall duro é decisão de produto pendente. Em dev (bypass de Stripe) o banner aparece pra toda academia criada, por design: reflete o estado real do banco.

**Pegadinha do estado de trial (jun/2026):** o aviso de fim de trial depende de `academies.subscription_status === 'trialing'` E `academies.trial_ends_at` preenchidos. Antes, o webhook (`checkout.session.completed`) e o `verify-session` gravavam `subscription_status: 'active'` fixo e nunca tocavam `trial_ends_at` (coluna existe desde a migration 001, mas ficava sempre null) — então starter/personal (que têm `trial_period_days: 30`) nunca eram marcados como em trial no banco. Corrigido: os dois agora fazem `stripe.subscriptions.retrieve()` e gravam o `status` real (`'trialing'` vs `'active'`) + `trial_ends_at` (de `sub.trial_end`). O handler `customer.subscription.updated` também passou a persistir `trial_ends_at`. **Quem grava `subscription_status`/`trial_ends_at` precisa vir do Stripe, não hardcodar `'active'`.** Banner lê só do store (`currentAcademy`), sem fetch extra. Pra testar em dev (sem Stripe real), setar manualmente `subscription_status='trialing'` + `trial_ends_at` ~2 dias à frente numa linha de `academies`.
...
**Colisão de número 072/075 resolvida (jun/2026):** o multi-dev via OneDrive recriou os números — havia DOIS `072` (`plan_limits` aplicada + `072_realtime_workout_sheets` nunca aplicada) e DOIS `075` (`contact_requests` aplicada + week_range nova). Mesma trava de `schema_migrations_pkey`. Fix: `realtime` renumerada pra `077` (conteúdo é idempotente — DO blocks de publicação + REPLICA IDENTITY — seguro reaplicar) e a nova week_range pra `076`. **Estado: prod sincronizado até a `077`.** Reforço da regra: ANTES de criar migration, `ls supabase/migrations | grep '^0NN'` pra achar o maior número real (a doc pode estar atrás do que o OneDrive sincronizou).

**Faixa de semanas nas fichas (jun/2026):** migration `076_workout_sheet_week_range.sql` adiciona `workout_sheets.week_start`/`week_end` (smallint nullable). É só periodização/organização — sem troca automática por calendário (decisão de produto). Personal define na criação (`treinos/novo`) e edita no detalhe (`treinos/[id]`, componente `WeekRangeCard`). Badge "Semana X-Y" via helper `formatWeekRange()` em `lib/utils.ts`, mostrado nos cards de `/treinos`, no detalhe e na lista de fichas do aluno (`alunos/[id]`).
