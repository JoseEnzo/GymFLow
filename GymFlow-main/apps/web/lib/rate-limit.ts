// Rate limiting REATIVADO com limites folgados (2026-06-11).
//
// Histórico: desativado em 2026-06-09 porque os limites originais (auth 5/15min)
// bloqueavam usuários legítimos com "Muitas tentativas". Reativado com limites
// ~4x mais generosos — proteção contra força-bruta sem atrito pra usuário real.
...
function createUpstashLimiters() {
  ...
  return {
    // Login, cadastro, recuperar-senha: 20 tentativas por 15 min por IP
    auth: new Ratelimit({
      redis,
    ...
    // Tokens de convite (lookup, accept, create): 30 por 5 min por IP
    ...
    // Rotas de API autenticadas: 60 por min por usuário
    api: new Ratelimit({
      redis,
...
export const limiters = upstash ?? {
  auth:   new InMemoryLimiter(20, 15 * 60 * 1000),  // 20/15min
  invite: new InMemoryLimiter(30,  5 * 60 * 1000),  // 30/5min
  api:    new InMemoryLimiter(60,       60 * 1000),  // 60/min
}
